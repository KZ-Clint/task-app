...
export default function Postsection ({comments, post}) {
  ...
  return (
    <>
       <div className='postsectionwrapper' >  
            <h1> { post?.info } </h1>
            <div className='postsectionwrapperfirsthead' >
                <div>
                    <h4> {post?.author} </h4>
                    <p> {post?.date} </p>
                    <span> <CommentIcon fontSize="medium" /> {comments.length} </span>
                </div>
                <img style={{ filter: `${ theme ? 'invert(1)' : 'invert(0)' }` }}  src={post?.img} alt="" />
            </div>
            <div className='postparagraphwrapper'  >
