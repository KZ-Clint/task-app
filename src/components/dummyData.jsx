export const breakingNews = [ 
    {
       info: "Chinese hackers gained access to some US government emails microsoft says.",
       date: "1 hour ago",
       category: "business"
    },
    {
        info: "Secret glass house for Elon musk? Tesla reportedly investigated millions of dollars funded by glass house companies.",
        date: "2 hour ago",
        category: "business"
     },
     {
        info: "PGA defends choice to back LIV deal in senate housing in senate hearing.",
        date: "3 hour ago",
        category: "sports"
     },
     {
        info: "Hours long caesers palace standoff ends in arrest by SWAT team.",
        date: "4 hour ago",
        category: "political"
     }
]

export const sportsNews = [ 
   {
      info: "Manchester United wins the treble for the second time in there history.",
      date: "1 hour ago",
      category: "sports",
      author: "Clark Ma",
      topstory:true,
      img:"/assets/manu.webp"
   },
   {
       info: "Fred kerley wins the 100m olympic gold in budapest.",
       date: "2 hour ago",
       category: "sports",
       author: "Jean Paul Hauge",
       topstory:false,
       img:"/assets/fkerley.webp"
    },
    {
       info: "PGA defends choice to back LIV deal in senate housing in senate hearing.",
       date: "3 hour ago",
       category: "sports",
       author: "Jay Jay Okocha",
       topstory:false,
       img:"/assets/pgatour.jpeg"
    },
    {
       info: "Neeraj Chopra wins the javelin throw in the final throw.",
       date: "4 hour ago",
       category: "sports",
       author: "Paulino Mark",
       topstory:false,
       img:"/assets/nchopra.jpg"
    },
    {
      info: "Serena williams qualify for the womens US opens.",
      date: "7 hour ago",
      category: "sports",
      author: "Mc smith",
      topstory:false,
      img:"/assets/serena.jpeg"
   },
   {
      info: "Chaos as talks of super league begins.",
      date: "7 hour ago",
      category: "sports",
      author: "John Doe",
      topstory:false,
      img:"/assets/superleague.webp"
   },
   {
      info: "Lebron James to announce his retirement soon.",
      date: "7 hour ago",
      category: "sports",
      author: "Kazmerack atinov",
      topstory:false,
      img:"/assets/lebron.jpg"
   },
   {
      info: "kipchoge wins 7th record marathon.",
      date: "10 hour ago",
      category: "sports",
      author: "Paul kurukame",
      topstory:false,
      img:"/assets/kipchoge.jpg"
   }
    
]

export const entertainmentNews = [ 
   {
      info: "Rema wins the 25th edition of the headies award.",
      date: "1 hour ago",
      category: "entertainment",
      author: "Clark Ma",
      topstory:false,
      img:"/assets/rema.jpg"
   },
   {
       info: "Beyonce and Jayz said to be having issues.",
       date: "2 hour ago",
       category: "entertainment",
       author: "Jean Paul Hauge",
       topstory:true,
       img:"/assets/beyjay.jpg"
    },
    {
       info: "Harry potter returning for a new movie sequel in 2024.",
       date: "3 hour ago",
       category: "entertainment",
       author: "Jay Jay Okocha",
       topstory:false,
       img:"/assets/harrypotter.jpg"
    },
    {
       info: "Avatar the last airbender to be shown live on HBC in early 2025.",
       date: "4 hour ago",
       category: "entertainment",
       author: "Paulino Mark",
       topstory:false,
       img:"/assets/avatar.jpg"
    },
    {
      info: "Top celebrities of 2023.",
      date: "7 hour ago",
      category: "entertainment",
      author: "Mc smith",
      topstory:false,
      img:"/assets/celebrities.jpg"
   },
   {
      info: "Why disney needs to promote more franchises?.",
      date: "7 hour ago",
      category: "entertainment",
      author: "John Doe",
      topstory:false,
      img:"/assets/disney.png"
   },
   {
      info: "Where does Netflix go from now?.",
      date: "7 hour ago",
      category: "entertainment",
      author: "Kazmerack atinov",
      topstory:false,
      img:"/assets/netflix.jpg"
   },
   {
      info: "Top blockbuster movies of 2023 so far.",
      date: "10 hour ago",
      category: "entertainment",
      author: "Paul kurukame",
      topstory:false,
      img:"/assets/blockbuster.webp"
   }
    
]

export const businessNews = [ 
   {
      info: "Tesla acquires electrical vehicle rival.",
      date: "1 hour ago",
      category: "business",
      author: "Clark Ma",
      topstory:false,
      img:"/assets/tesla.jpg"
   },
   {
       info: "Adobe completes acquisition of figma.inc.",
       date: "2 hour ago",
       category: "business",
       author: "Jean Paul Hauge",
       topstory:true,
       img:"/assets/adobe.jpg"
    },
    {
       info: "Companies to look out for in 2023.",
       date: "3 hour ago",
       category: "business",
       author: "Jay Jay Okocha",
       topstory:false,
       img:"/assets/topcomp.png"
    },
    {
       info: "Top startups to watch in 2024.",
       date: "4 hour ago",
       category: "business",
       author: "Paulino Mark",
       topstory:false,
       img:"/assets/topstartups.jpg"
    },
    {
      info: "Walmart looking to expand as it forsees growth in the future.",
      date: "7 hour ago",
      category: "business",
      author: "Mc smith",
      topstory:false,
      img:"/assets/walmart.jpg"
   },
   {
      info: "African businesses looking for funding, slow growth inevitable.",
      date: "7 hour ago",
      category: "business",
      author: "John Doe",
      topstory:false,
      img:"/assets/afribiz.jpg"
   },
   {
      info: "Amazon and its business model proving freightning for regulators.",
      date: "7 hour ago",
      category: "business",
      author: "Kazmerack atinov",
      topstory:false,
      img:"/assets/amazon.png"
   },
   {
      info: "Key sectors that have fallen far below .",
      date: "10 hour ago",
      category: "business",
      author: "Paul kurukame",
      topstory:false,
      img:"/assets/compfallen.jpeg"
   }
    
]


